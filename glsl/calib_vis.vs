...
void main() {
  ...
  // geo_pos_world  = (vec4(pos_vol, 1.0)).xyz;

  geo_pos_view   = (gl_ModelViewMatrix * vec4(geo_pos_world, 1.0)).xyz;
  // geo_distance   = texture(cv_uv[layer], pos_calib).xy;
  // geo_distance   = texture(cv_uv_inv[layer], geo_pos_volume).xy;
  
  geo_distance = texture(volume_tsdf, geo_pos_volume).r;
